...
router.get("/get", [
    ...
    isInt().withMessage("purchaseYearFilter is not a number").toInt(),
    query("lowSellDateFilter").trim().default(null).optional({checkFalsy:true}).
    isDate().withMessage("lowSellDateFilter is not valid date format").toDate(),
    query("highSellDateFilter").trim().default(null).optional({checkFalsy:true}).
    isDate().withMessage("highSellDateFilter is not valid date format").toDate(),
    ...
    isInt().withMessage("limit must be a int value").toInt()
...
router.post("/insert", [
    ...
    exists({checkFalsy:true}).withMessage("serialNumber is required").
    isLength({min:1, max:20}).withMessage("serialNumber must be less or equal 20 characters"),
    body("landNumber").trim().
    exists({checkFalsy:true}).withMessage("landNumber is required").
    isLength({min:1, max:7}).withMessage("serialNumber must be less or equal 20 characters"),
    body("area").trim().
    exists({checkFalsy:true}).withMessage("area is required").
    isFloat({min:0, max:9999}).withMessage("area must be a float value less than 10000").toFloat(),
    body("town").trim().
    exists({checkFalsy:true}).withMessage("town is required").
    isLength({min:1, max:50}).withMessage("town must be less or equal 50 characters"),
    body("commune").trim().
    exists({checkFalsy:true}).withMessage("commune is required").
    isLength({min:1, max:50}).withMessage("commune must be less or equal 50 characters"),
    body("district").trim().
    exists({checkFalsy:true}).withMessage("district is required").
    isLength({min:1, max:50}).withMessage("district must be less or equal 50 characters"),
    body("province").trim().
    exists({checkFalsy:true}).withMessage("province is required").
    isLength({min:1, max:50}).withMessage("province must be less or equal 50 characters"),
    body("idOwner").trim().
    ...
    isLength({min:1, max:15}).withMessage("registerNumber must be less or equal 15 characters"),
    body("mortgage").trim().default(false).optional().
    ...
    isLength({min:1, max:65535}).withMessage("description must be less or equal 65535 characters"),
    ...
    isLength({min:1, max:21}).withMessage("purchaseActNumber must be less or equal 21 characters"),
    body("seller").trim().default(null).optional({checkFalsy:true}).
    isLength({min:1, max:50}).withMessage("seller must be less or equal 50 characters"),
    ...
    isLength({min:1, max:21}).withMessage("sellActNumber must be less or equal 21 characters"),
    body("buyer").trim().default(null).optional({checkFalsy:true}).
    isLength({min:1, max:50}).withMessage("buyer must be less or equal 50 characters"),
