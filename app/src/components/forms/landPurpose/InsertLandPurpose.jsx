...
const InsertLandPurpose = ({onClose = () => {}, reload = () => {}}) => {
    const {post} = useApi();
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if(isValidated()) {
            post("/api/land-purposes/insert", {...fieldData}, (res) => {
                onClose()
                reload()
            });
        }
    ...
    return (
        <form onSubmit={handleSubmit} className="w-[33%] border-l-4 border-l-green-700 p-5 flex flex-col items-center scroll-auto">
            ...
            <h1 className="text-2xl font-bold">Dodaj Przeznaczenie diałki</h1>
            <section className="my-4 gap-y-2 flex flex-col w-[80%]">
                <Input
                    placeholder="Podaj przeznaczenie działki"
...
export default InsertLandPurpose;