...
const UpdateGeneralPlan = ({onClose = () => {}, reload = () => {}, generalPlans = []}) => {
    const {put} = useApi();
    const generalPlanData = useUpdateDataStore((state) => state.data);
    ...
    useEffect(() => {
        if(fieldData.code) {
            const obj = [...generalPlans].find((obj) => obj.code === fieldData.code);
            if (obj) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if(isValidated()) {
            put("/api/general-plans/update", {idGeneralPlan:generalPlanData.id, ...fieldData}, (res) => {
                onClose()
                reload()
            });
        }
    ...
    return (
        <form onSubmit={handleSubmit} className="w-[33%] border-l-4 border-l-green-700 p-5 flex flex-col items-center scroll-auto">
            ...
            <h1 className="text-2xl font-bold">Edycja Planu ogólnego</h1>
            <h2 className="text-2xl font-bold">Nr {generalPlanData.number}</h2>
...
export default UpdateGeneralPlan;